...
function MainPhoto() {
  ...
  const gridPressed = value => {
    ...
    return <Content photoList={photoList} grid={grid} />;
  };
  const sequencePressed = value => {
    sequence === 'new' ? setSequence('old') : setSequence('new');
    return <Content photoList={photoList} sequence={sequence} />;
  };
  return (
    <ImageBackground source={require(backgroundImg)} style={styles.container}>
      <SafeAreaView style={styles.safeAreaViewContainer}>
        <Header gridPressed={gridPressed} sequencePressed={sequencePressed} />
